...
import re
from data_models import Billboard
...
def get_year_end_hot_100_songs(years: list) -> list[tuple]:
    ...
    for year in years:
        ...
        ranks = container_element.find_all(
            "span", class_=re.compile(r".*c-label a-font-primary-bold-l.*")
        )
        artists = container_element.find_all(
            "span", class_=re.compile(r".*c-label a-font-primary-s.*")
        )
        songs = container_element.find_all("h3", id="title-of-a-story")

        # collect data
        for rank, artist, song in zip(ranks, artists, songs):
            rank_text, artist_text, song_text = (
                rank.text.strip(),
                artist.text.strip(),
                song.text.strip(),
            )
            ...
            collected_data.append(Billboard(**billboard_data))
